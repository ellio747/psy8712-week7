...
# Visualization
week7_tbl %>% 
  ggpairs(columns = 5:13, lower = list(continuous = "points"), upper = list(continuous = "cor"), diag = list(continuous = "densityDiag"))
ggsave(filename = "../figs/ggally.fig.png", height=3, width=4, units="in", dpi=600)
ggplot(week7_tbl, aes(timeStart, q1)) + 
  geom_point() +
  xlab("Date of Experiment") +
  ylab("Q1 Score") +
  labs(title = "Fig 1")
ggsave(filename = "../figs/fig1.png", height=3, width=4, units="in", dpi=600)
ggplot(week7_tbl, aes(q1, q2, color = gender)) +
  geom_point(position = "jitter") +
  labs(title = "Fig 2")
ggsave(filename = "../figs/fig2.png", height=3, width=4, units="in", dpi=600)
...
ggplot(week7_tbl, aes(q1, q2)) +
  ...
  ylab("Score on Q2") +
  labs(title = "Fig 3")
ggsave(filename = "../figs/fig3.png", height=3, width=4, units="in", dpi=300)
ggplot(week7_tbl, aes(gender, timeSpent)) +
  geom_boxplot() +
  xlab("Gender") +
  ylab("Time elapsed (mins)") +
  labs(title = "Fig 4")
ggsave(filename = "../figs/fig4.png", height=3, width=4, units="in", dpi=600)
ggplot(week7_tbl, aes(q5, q7, color = condition)) + 
  ...
  ylab("Score on Q7") +
  labs(title = "Fig 5")
ggsave(filename = "../figs/fig5.png", height=3, width=4, units="in", dpi=600)
